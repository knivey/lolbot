...
class PathSegmentTest extends TestCase
{
    ...
    private function flattenSeg(PathSegment $seg, float $startX, float $startY, float $tolerance): array
    {
        $budget = RenderLimits::maxPathVertices;
        return $seg->flatten($startX, $startY, $tolerance, $budget);
    }

    public function test_move_to_flatten_returns_empty(): void
    {
        $seg = new MoveTo(10.0, 20.0);
        $this->assertSame([], $this->flattenSeg($seg,0.0, 0.0, 0.5));
    }
    ...
    public function test_line_to_flatten_returns_endpoint(): void
    {
        ...
        $result = $this->flattenSeg($seg,5.0, 5.0, 0.5);
    ...
    public function test_close_path_flatten_returns_empty(): void
    {
        ...
        $this->assertSame([], $this->flattenSeg($seg,30.0, 40.0, 0.5));
    }
    ...
    public function test_quadratic_bezier_straight_line_flattens_to_one_vertex(): void
    {
        ...
        $result = $this->flattenSeg($seg,0.0, 0.0, 0.5);
        $this->assertCount(1, $result);
    ...
    {
        $seg = new QuadraticBezier(0.0, 10.0, 10.0, 0.0);
        $fine = $this->flattenSeg($seg,0.0, 0.0, 0.1);
        $coarse = $this->flattenSeg($seg,0.0, 0.0, 2.0);
    ...
    public function test_arc_large_arc_flag(): void
    {
        ...
        $smallResult = $this->flattenSeg($small, 0.0, 0.0, 0.5);
        $largeResult = $this->flattenSeg($large, 0.0, 0.0, 0.5);
        $this->assertGreaterThan(
            count($smallResult),
            count($largeResult),
