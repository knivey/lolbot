...
class github extends script_base
{
    ...
    function handleEvents(UrlEvent $event): void
    {
        ...
        $event->promises[] = \Amp\call(function () use ($event) {
            ...
            if (preg_match("@^(?:www\.)?github\.com$@i", $uri->getHost())) {
                ...
                if ($repoAction == null) {
                    if ($out = yield $this->github($user, $repo)) {
                        $event->reply($out);
                        return;
                    }
                }
                ...
                if (strtolower($repoAction) == 'issues' && isset($repoParts[0])) {
                    if ($out = yield $this->github_issueStr($user, $repo, $repoParts[0])) {
                        $event->reply($out);
                        return;
                    }
                }
    ...
    function github_cmd($args, \Irc\Client $bot, \knivey\cmdr\Args $cmdArgs)
    {
        ...
        $repo = $m[2] ?? null;
        try {
            $out = yield $this->github($user, $repo);
            if (!$out) {
...
//TODO throw exceptions instead of just return false
    ...
    function github($user, $repo)
    {
        return \Amp\call(function () use ($user, $repo) {
            if ($repo != null)
                return yield $this->github_repo($user, $repo);
            $url = "https://api.github.com/users/$user";
            if ($json = yield $this->github_get_json($url, true)) {
                $hireable = "false";
                if ($json['hireable'])
                    $hireable = "true";
                $created = $this->reftime($json["created_at"]);
                $stars = yield $this->github_stars($user);
    ...
    function github_issueStr($user, $repo, $issue)
    {
        return \Amp\call(function () use ($user, $repo, $issue) {
            ...
            if (!$json = yield $this->github_get_json($url)) {
                return false;
            }
    ...
    function github_repo($user, $repo)
    {
        return \Amp\call(function () use ($user, $repo) {
            ...
            $body = yield $this->github_get_json($url, true);
            $langs = yield $this->github_langs($user, $repo);
            if (!$body)
                return false;
